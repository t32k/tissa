...
exports.SETTINGS = (function () {
	...
	var view = Ti.UI.createView(TA.UI.bgLinen);
	...
	var notifySwitch = Ti.UI.createSwitch({
		right: 10,
		value: TA.notification
	});
	notifyRow.add(notifySwitch);
	timerSection.add(notifyRow);
	notifySwitch.addEventListener('change', function(e) {
    TA.notification = e.value;
    Ti.App.Properties.setInt('Notification', TA.notification);
	});
	...
	TA.addRow = function (index) {
		...
		var txt = (index !== '01') ? ' mins' : ' min',
			row = Ti.UI.createPickerRow(),
			view = Ti.UI.createView(TA.UI.pickerRow.view),
			num = Ti.UI.createLabel(TA.extend(TA.UI.pickerRow.num, {
				text: index
			})),
	...
	var footerLabel = Ti.UI.createLabel({
		height: 48,
		text: 'tissa for iOS Version ' + Ti.App.version + '\n © 2011 i&M',
		textAlign: 'center',
		font: {
			fontSize: 12,
		},
	...
	// Initialization
	TA.tableView = Ti.UI.createTableView(TA.extend(TA.UI.table, {
		data: TA.data.main
	}));
	view.add(TA.tableView);
	win.add(view);
	...
	TA.tableView.addEventListener('click', function (e) {
		switch (e.index) {
		case 0:
			nav.open(dispWin, {
				animated: true
